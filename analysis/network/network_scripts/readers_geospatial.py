...
import rioxarray
from constants import CRS, PROV_RENAME_MAP
...
def read_admin2_shapes(
    path: str, fix=True, exclude=["Macau", "HongKong"], sort=True
) -> gpd.GeoDataFrame:
    ...
    admin_l2 = admin_l2.drop_duplicates(subset=["NL_NAME_2", "NAME_1"]).copy()
    admin_l2 = admin_l2.drop_duplicates(subset=["NAME_2", "NAME_1"]).copy()
    admin_l2.reset_index(drop=True, inplace=True)
    if fix:
        admin_l2 = _fix_gadm41(admin_l2)

    if sort:
        admin_l2 = admin_l2.sort_values(["NAME_1", "NAME_2"])
