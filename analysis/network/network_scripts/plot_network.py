...
from _pypsa_helpers import (
    compute_transmission_utilisation,
    filter_carriers,
    unscale_biomass_network,
)
...
def prepare_edge_data(
    ...
    def _normalize_widths(capacities: pd.Series) -> pd.Series:
        """Set sub-threshold capacities to 0, then scale by ``linewidth_factor``.

        The previous implementation used ``clip(lower=min) → replace(min, 0)``,
        which also wiped out lines whose capacity was *exactly* ``min_edge_capacity``
        (and silently floored all sub-threshold lines to the threshold before
        replacing). Using ``where`` keeps the actual capacity for above-threshold
        lines and writes 0 for the rest.
        """
        if capacities.empty:
            return capacities.astype(float)
        capacities = pd.to_numeric(capacities, errors="coerce").fillna(0.0)
        widths = capacities.where(capacities >= min_edge_capacity, 0.0)
    ...
    line_caps = network.lines.s_nom if additions else network.lines.s_nom_opt
...
def prepare_network_plot_data(
    ...
    # Expose the resolved factor so the legend reference swatches use the SAME
    # scaling as the plotted edges (otherwise the legend defaults to a
    # different factor and the reference width misrepresents capacity).
    result["linewidth_factor"] = linewidth_factor
...
def plot_map(
    ...
    **kwargs,
    ...
    if add_ref_edge_sizes and ref_edge_color is not None and _ref_edge_sizes:
        ...
        # Must match prepare_edge_data's default (6e3) so reference swatches
        # scale identically to the plotted edges. Callers thread the resolved
        # factor through via plot_data["linewidth_factor"].
        size_factor = float(kwargs.get("linewidth_factor", 6e3))
...
def plot_network(
    ...
    **kwargs,
    ...
    ax = plot_map(
        ...
        # Same factor used to scale the plotted edges, so the legend reference
        # swatches represent capacity at the same scale as the drawn lines.
...
def plot_line_utilisation_map(
    ...
    """Plot transmission utilisation (capacity factor) as an edge colormap.

    Colours every transmission edge by `compute_transmission_utilisation` (0..1):
    OSM AC `n.lines` AND transmission `n.links` (manual-backend AC links + all
    HVDC corridors). Split positive+reversed legs are folded to one value per
    corridor; reversed/sector legs and unplaced (0,0) buses are hidden (width 0).
    Edge widths scale with optimised capacity, so thick+bright = high-capacity,
    heavily-used corridors.

    Args:
        n: Solved PyPSA network.
        plot_config: Plotting config (reads `projection`, `figsize`).
        save_path: PNG output path.
        admin2_shapes: Optional pre-loaded admin2 shapes for the China extent.
        cmap_name: Matplotlib colormap name.

    Returns:
        (figure, axes).
    """
    util = compute_transmission_utilisation(n)
    cmap = plt.get_cmap(cmap_name)
    norm = mcolors.Normalize(vmin=0.0, vmax=1.0)
    grey = "#dddddd"  # edges with no utilisation value (reversed/sector legs)

    def _colors(u: pd.Series, index: pd.Index) -> pd.Series:
        vals = u.reindex(index)
        return pd.Series(
            [grey if pd.isna(v) else mcolors.to_hex(cmap(norm(v))) for v in vals], index=index
        )

    def _widths(caps: pd.Series, keep: pd.Series, floor: float = 0.0) -> pd.Series:
        w = (pd.to_numeric(caps, errors="coerce").fillna(0.0) / 3e3).clip(floor, 6.0)
        return w.where(keep.values, 0.0)

    if len(n.lines):
        line_colors = _colors(util["lines"], n.lines.index)
        line_widths = _widths(n.lines.s_nom_opt, pd.Series(True, index=n.lines.index), floor=0.3)
    else:
        line_colors = pd.Series(dtype=object)
        line_widths = pd.Series(dtype=float)

    if len(n.links):
        link_u = util["links"]
        link_colors = _colors(link_u, n.links.index)
        # Keep only real transmission legs (util not NaN) with placed endpoints.
        placed = (n.links.bus0.map(n.buses["x"]) != 0) & (n.links.bus1.map(n.buses["x"]) != 0)
        keep = link_u.reindex(n.links.index).notna() & placed
        link_widths = _widths(n.links.p_nom_opt, keep)
    else:
        link_colors = pd.Series(dtype=object)
        link_widths = pd.Series(dtype=float)

    projection = get_projection(plot_config.get("projection", "PlateCarree"))
    fig, ax = plt.subplots(subplot_kw={"projection": projection})
    fig.set_size_inches(list(plot_config.get("figsize", [10, 8])))
    ...
    # Extent from bus coordinates (self-contained; avoids shapefile dependency).
    try:
        apply_china_extent(ax, admin2_shapes, margin=plot_config.get("china_margin", 2.2))
    except Exception:
        m = plot_config.get("china_margin", 2.2)
        bx, by = n.buses["x"], n.buses["y"]
        # Ignore unplaced/converter buses at (0, 0) so they don't blow up the extent.
        valid = (bx > 70) & (bx < 140) & (by > 5) & (by < 60)
        if valid.any():
            ax.set_extent(
                [bx[valid].min() - m, bx[valid].max() + m, by[valid].min() - m, by[valid].max() + m],
                crs=ccrs.PlateCarree(),
            )
        else:
            ax.set_extent([73, 136, 15, 54], crs=ccrs.PlateCarree())
    add_gridlines(ax)

    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, orientation="vertical", shrink=0.6, pad=0.02)
    cbar.set_label("Transmission utilisation [-]")
    ax.set_title("Transmission utilisation (snapshot-weighted capacity factor)")

    if save_path:
        fig.savefig(save_path, bbox_inches="tight")
    logger.info("Saved line-utilisation map to %s", save_path)
    return fig, ax
...
if __name__ == "__main__":
    ...
    # ── Line-utilisation map (AC lines coloured by capacity factor) ───────────
    util_path = snakemake.output.cost_map.replace("cost.png", "line_utilisation.png")
    plot_line_utilisation_map(n, plot_config, save_path=util_path)
