...
Uso:
  python analysis/capacity/master_noCO2_analysis.py
"""
...
def extract_metrics(network_file):
    ...
    # Monthly load
    load_ts = n.loads_t.p_set.sum(axis=1) #tabella datraframe con indice snapshot e colonna p_set (carico totale in ogni snapshot)
    load_monthly = (load_ts * w).resample("M").sum() / 1e6 #resample per mese, somma e conversione in TWh
...
def plot_load_comparison(metrics, output_dir):
    ...
    ax.legend(handles=[
        Patch(facecolor="#888888", alpha=0.85, label="Model (noCO2)"),
    ...
    ax.set_title("Total electricity demand — Model (noCO2) vs reference\n"
...
def plot_capacity_comparison(metrics, output_dir):
    ...
    plt.suptitle("Installed capacity by carrier — Model (noCO2) vs reference",
                 fontsize=13, y=1.02)
    plt.tight_layout()
...
def plot_generation_comparison(metrics, output_dir):
    ...
    plt.suptitle("Electricity generation by carrier — Model (noCO2) vs reference",
                 fontsize=13, y=1.02)
    plt.tight_layout()
...
def plot_generation_shares(metrics, output_dir):
    ...
    plt.suptitle("Generation mix — Model (noCO2) scenarios", fontsize=13)
    plt.tight_layout()
...
def plot_capacity_trajectory(metrics, output_dir):
    ...
    ax.set_title("Capacity trajectory — Model (noCO2): 2020 → 2025 → 2060", fontsize=12)
    ax.set_xticks(years)
...
if __name__ == "__main__":
    ...
    log("=" * 70)
    log("MASTER noCO2 ANALYSIS — CN2020 / CN2025 / CN2060 Admin2")
    log("=" * 70)
